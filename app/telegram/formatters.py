...
def format_official_daily_digest(payloads: list[dict], date: str) -> str:
    """Forecasting 2.0: the daily push digest of official 24h forecasts
    just generated by generate_official_daily_forecast_job -- one line per
    symbol that actually produced a forecast this cycle (a symbol with
    insufficient data that cycle is simply absent, never a fabricated
    placeholder line). `payloads` are the exact dicts ForecastEngine.compute()
    already returned this cycle, not a re-fetch."""
    if not payloads:
        return (
            f"*OFFICIAL DAILY FORECAST -- {date}*\n\n"
            "No official forecasts could be generated this cycle "
            "(insufficient data for every configured symbol)."
        )
    lines = [f"*OFFICIAL DAILY FORECAST -- {date}*", ""]
    for p in payloads:
        lines.append(
            f"{p['symbol']}: {p['direction']} {p['probability_pct']}% -- "
            f"{p['current_price']:,.2f} -> {p['target_price']:,.2f} "
